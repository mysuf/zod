...
function handleCatchall(
  ...
  for (const key in input) {
    ...
    if (r instanceof Promise) {
      proms.push(r.then((r) => handlePropertyResult(r, payload, key, input, isOptionalIn, isOptionalOut, ctx.greedy)));
    } else {
      handlePropertyResult(r, payload, key, input, isOptionalIn, isOptionalOut, ctx.greedy);
    }
  }
...
export const $ZodObject: core.$constructor<$ZodObject> = /*@__PURE__*/ core.$constructor("$ZodObject", (inst, def) => {
  ...
  inst._zod.parse = (payload, ctx) => {
    ...
    for (const key of value.keys) {
      ...
      if (r instanceof Promise) {
        proms.push(
          r.then((r) => handlePropertyResult(r, payload, key, input, isOptionalIn, isOptionalOut, ctx?.greedy))
        );
      } else {
        handlePropertyResult(r, payload, key, input, isOptionalIn, isOptionalOut, ctx?.greedy);
      }
